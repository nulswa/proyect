   ...
   try {
      ...
      if (body && !isNaN(body) && global.typo.has(m.sender)) {
         ...
         if (session.commands && session.commands[choice]) {
            ...
            await client.reply(m.chat, `Probando *${prefix + command}*...`, m)
         }
      }

      const isSpam = spam.detection(client, m, {
         prefix, command, commands, users, cooldown,
         show: 'all', // options: 'all' | 'command-only' | 'message-only' | 'spam-only'| 'none' :v
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `Estás de vuelta en línea después de haber estado desconectado durante : ${Utils.texted('bold', Utils.toTime(new Date - users.afk))}\n\n• Razón : ${users.afkReason ? users.afkReason : '×'}`, m)
         users.afk = -1
         users.afkReason = ''
         users.afkObj = {}
      }
      ...
      if (prefix && !commands.includes(command) && matcher.length > 0 && !setting.self) {
         if (!m.isGroup || (m.isGroup && !groupSet.mute)) {
            ...
            let caption = `📍  *No se han encontrado comandos relacionados.*\n- Responda a este mensaje con el número para probar.\n\n`
            caption += global.typo.get(m.sender).commands.map((v, i) => `*${i + 1}* » ${prefix + v} de ${matcher[i].accuracy}%`).join('\n')
      ...
      if (
         ...
         if (setting.error.includes(command)) return client.reply(m.chat, `📍  El comando *${(prefix ? prefix : '') + command}* esta desactivado.`, m)
         ...
         for (const [pluginPath, pluginData] of Object.entries(is_commands)) {
            ...
            if (!m.isGroup && !['owner', 'menfess', 'scan', 'verify', 'payment', 'premium'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) {
               client.reply(m.chat, 'Lo siento, no puedes usar el bot en el chat individual.\n- Puedes solicitar un paquete premium para usar el modo privado.', m).then(() => chats.lastchat = new Date() * 1)
               continue
            }
            if (!['me', 'owner', 'exec'].includes(name) && users && (users.banned || new Date - users.ban_temporary < Config.timeout)) {
               client.reply(m.chat, `${isSpam.msg}`, m)
               continue
            }
            ...
            if (cmd.restrict && !isPrem && !isOwner && text && new RegExp('\\b' + setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `📍  Violaste los términos y condiciones de uso, serás baneado por haber incumplido la regla.`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               client.reply(m.chat, `No tienes más usos gratuitos, vuelva mañana.`, m).then(() => users.premium = false)
               continue
            }
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, `No tienes usos gratuitos para usar está función.`, m)
                  continue
               }
            }
         ...
         for (const [pluginPath, pluginData] of Object.entries(is_events)) {
            ...
            if (!m.isGroup && setting.groupmode && !['system_ev', 'menfess_ev', 'chatbot', 'auto_download'].includes(name) && !isPrem) return client.reply(m.chat, `No puedes usar el bot en el chat individual.\n- Puedes entrar al grupo principal o alojarlo.`, m).then(() => chats.lastchat = new Date() * 1)
            ...
            if (event.limit && !event.game && users.limit < 1 && body && Utils.generateLink(body) && Utils.generateLink(body).some(v => Utils.socmed(v))) return client.reply(m.chat, `No tienes *límites* restantes, vuelva mañana para restablecer tus limites.`, m).then(() => {
               users.premium = false
               users.expired = 0
            })
