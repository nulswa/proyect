...
export const run = {
   ...
   async: async (m, { client, Utils, setting }) => {
      try {
         ...
         await client.sendReact(m.chat, setting.timeLaod, m.key)
         ...
         if (m.quoted && m.quoted?.message) {
            ...
            if (/(image|video)/.test(type)) {
               ...
               exec(`ffmpeg -i ${media} -vn -ar 44100 -ac 2 -b:a 128k ${result}`, async (err, stderr, stdout) => {
                  remove(media)
                  if (err) return client.reply(m.chat, `${setting.errorBot}`, m)
                  let buff = read(result)
                  client.sendFile(m.chat, buff, 'audio.mp3', '', m).then(() => {
                     remove(result)
                  })
               })
            } else client.reply(m.chat, `${setting.emoji2}  Responda a un archivo *ViewOnce*.`, m)
         ...
         } else client.reply(m.chat, `${setting.emoji2}  Debes responder a un archivo *ViewOnce*.`, m)
      } catch (e) {
