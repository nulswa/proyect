         ...
         if (id.endsWith('g.us')) {
            ...
            for (let sender in presences) {
               ...
               if ((presence.lastKnownPresence === 'composing' || presence.lastKnownPresence === 'recording') && user.afk > -1) {
                  sock.reply(id, `El sistema detecto la actividad de @${user.jid.replace(/@.+/, '')} durante su inactividad en : ${Utils.texted('bold', Utils.toTime(new Date - (user?.afk || 0)))}\n\n> *Razón :* ${user?.afkReason || '×'}`, user?.afkObj)
                  user.afk = -1
                  user.afkReason = ''
                  user.afkObj = {}
               }
      ...
      client.register('group.add', async ctx => {
         try {
            ...
            if (groupSet && groupSet.localonly) {
               if (global.db.users.some(v => v.jid === memberId) && !global.db.users.find(v => v.jid == memberId).whitelist && !memberId.startsWith('54') || !memberId.startsWith('54')) {
                  sock.reply(ctx.jid, `🌾  Lo siento @${memberId.split`@`[0]}, este grupo solo es para la gente de *Argentina*, serás eliminado de inmediato.`)
